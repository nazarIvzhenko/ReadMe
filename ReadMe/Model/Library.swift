...
//  ReadMe
//
//  Created by Nazar Ivzhenko on 05.06.2022.
//
...
enum LibrarySymbol {
    ...
    var image: UIImage {
        ...
        case .letterSquare(let letter):
            guard let letter = letter?.lowercased(), let image = UIImage(systemName: "\(letter).square") else {
                imageName = "square"
                break
            }
            return image
...
enum Library {
    private static let starterData = [
        Book(title: "the War of Art", author: "Steven Pressfield", readMe: true),
        Book(title: "Can't Hurt Me", author: "David Goggins", readMe: true),
        Book(title: "Essentialism", author: "Greg McKeown", readMe: true),
        Book(title: "Atomic Habits", author: "James Clear", readMe: false),
        Book(title: "How to Win Friends & Influence People", author: "Dale Carnegie", readMe: false)
    ]
